...
import com.knoxhack.betteragriculture.client.model.ModelEntityMobSheep1Model1;
    ...
    public void doRenderLayer(EntityMobSheep1 entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (!entitylivingbaseIn.getSheared() && !entitylivingbaseIn.isInvisible())
        {
            ...
            if (entitylivingbaseIn.hasCustomName() && "jeb_".equals(entitylivingbaseIn.getCustomNameTag()))
            {
                ...
                float[] afloat1 = EntityMobSheep1.getDyeRgb(EnumDyeColor.byMetadata(k));
                float[] afloat2 = EntityMobSheep1.getDyeRgb(EnumDyeColor.byMetadata(l));
